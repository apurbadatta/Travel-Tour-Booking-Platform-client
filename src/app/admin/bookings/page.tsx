      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Monitor bookings, track total payments, and transaction records.</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          {filteredBookings.length === 0 ? (
            <div className="p-16 text-center text-slate-500">
              <p className="font-semibold text-lg text-slate-700">No bookings found</p>
                    ...
                    <tr key={item._id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 min-w-[200px]">
                        <div className="flex items-center space-x-3">
                          <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200 flex-shrink-0">
                            {item.user?.image ? (
                          ...
                          <div>
                            <span className="font-bold text-slate-900 block">{item.user?.name || 'Unknown User'}</span>
                            <span className="text-xs text-slate-500 block">{item.user?.email || 'N/A'}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 min-w-[200px]">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <span className="font-semibold text-slate-900 block line-clamp-1">{item.tour?.title || 'Unknown Tour'}</span>
                      ...
                      <td className="px-6 py-4">
                        {item.paymentStatus === 'paid' ? (
                          <div className="space-y-1">
                            <div className="flex items-center gap-1 text-xs text-slate-700 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 w-fit max-w-[150px]">
                            ...
                            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block ml-1">
